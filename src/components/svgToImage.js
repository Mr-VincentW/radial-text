...
export default (canvas, options) => new Promise((resolve, reject) => {
  options = options || {};

  const targetCanvas = canvas.cloneNode(true),
    canvasStyles = getComputedStyle(canvas),
    outputType = /^(?:(?:DATA|OBJECT)_URL|BLOB)$/i.test(options.type) ? RegExp.$_.toUpperCase() : 'OBJECT_URL',
    imgFormat = 'image/' + (/^(png|jpe?g|webp)$/i.test(options.imgFormat) ? RegExp.$_.toLowerCase().replace('jpg', 'jpeg') : 'png'),
    imgQuality = imgFormat === 'image/png' ? undefined : (isNaN(options.imgQuality) ? undefined : Math.min(Math.max(parseInt(options.imgQuality), 1), 100)) / 100;

  targetCanvas.style.visibility = 'hidden';
  !canvasStyles.background && imgFormat === 'image/jpeg' && (targetCanvas.style.background = '#FFFFFF');
  document.body.appendChild(targetCanvas);

  const canvasBBox = targetCanvas.getBBox();
  targetCanvas.setAttribute('viewBox', `${canvasBBox.x} ${canvasBBox.y} ${canvasBBox.width} ${canvasBBox.height}`);
  targetCanvas.remove();
  targetCanvas.style.visibility = '';

  if (typeof window.URL === 'function' && typeof window.Promise === 'function') {
    var canvasImgSrc = URL.createObjectURL(new Blob([new XMLSerializer().serializeToString(targetCanvas)], {
    ...
    new Promise(resolve => {
      ...
      canvasImg.onerror = () => reject('An error occurred when creating the image.');
      ...
      const imgOverSizedErrorMsg = 'The image may be too large, please try reducing its dimentions.',
        imgInfo = {
      ...
      if (outputType === 'DATA_URL') {
        ...
        dataURL.length < 10 ? reject(imgOverSizedErrorMsg) : resolve({
          url: dataURL,
          ...imgInfo
        });
      } else {
            ...
            if (blob) {
              resolve({
                ...(outputType === 'BLOB' ? {
                  ...
                  url: outputPaintObjectUrl = URL.createObjectURL(blob)
                }),
                ...imgInfo
              });
            } else {
