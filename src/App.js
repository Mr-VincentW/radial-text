...
export default function App() {
  ...
  const colorPickerOnClick = useCallback(e => e.target.previousSibling.click(), []);
  ...
  const imgTypeSelectorOnChange = useCallback(e => {
    const imgQualitySettingItem = document.querySelector('.img-quality-setting'),
      imgQualitySlider = imgQualitySettingItem.nextSibling.querySelector('input');

    if (e.target.value === 'png') {
      imgQualitySettingItem.classList.add('disabled');
      imgQualitySettingItem.nextSibling.setAttribute('title', 'Not applicable to PNG type');
  ...
  const saveBtnOnClick = useCallback(e => {
    ...
    getPaint(canvasRef.current, {
      ...
      if (isActionCopy) {
        ...
        downloadLink.setAttribute('download', `RadialText_${new Date().getTime()}.${settings.imgType || 'png'}`);
  ...
  useEffect(() => {
    ...
    let canvasBBox = canvasRef.current.getBBox(),
      canvasScale = /scale\((.*?)\)/.test(canvasRef.current.querySelector('.text-lines').getAttribute('transform')) ? parseFloat(RegExp.$1) : 1;

    canvasScale = 1 / (isNaN(canvasScale) ? 1 : canvasScale);
    setEstimatedImgDimentions(`${Math.round(canvasBBox.width * canvasScale)} × ${Math.round(canvasBBox.height * canvasScale)}`);
  }, [settings]);
  ...
  return (
    <div className="app">
      <form className="settings" onChange={settingsOnChange}>
        <dl>
          ...
          <dd className="through">
            <textarea name="textLines" placeholder="Leading and trailing spaces will be auto-trimmed."></textarea>
            <div className="line-setting">
              <label>
                <input name="ignoreEmpty" type="checkbox" defaultChecked={true} />
                <span>Ignore empty lines</span>
              </label>
              <label><span>Count: </span><i>{textLinesCount}</i></label>
            </div>
          </dd>
          </dl>
          <dl>
          ...
          <dd>
            <label>
              <input name="fontSize" placeholder={16} type="number" defaultValue={16} min={0} />
              <span>px</span>
            </label>
          </dd>
          <dt>Font Weight:</dt>
          ...
          <dd>
            <label>
              <input name="lineHeight" placeholder={1} type="number" defaultValue={1} min={0} />
          ...
          <dd>
            <label>
              <input name="centricCircleRadius" placeholder="auto" type="number" min={0} />
              <span>px</span>
            </label>
          </dd>
          ...
          <dd>
            <label>
              <input type="radio" name="color" value="SPECTRUM" checked={!settings.color} onChange={colorRadioOnChange} />
              <span>SPECTRUM</span>
            </label>
            <label>
              <input type="radio" name="color" value="" checked={!!settings.color} onChange={colorRadioOnChange}/>
          ...
          <dd>
            <label>
              <input type="radio" name="bgColor" value="TRANSPARENT" checked={!settings.bgColor} onChange={colorRadioOnChange} />
              <span>TRANSPARENT</span>
            </label>
            <label>
              <input type="radio" name="bgColor" value="" checked={!!settings.bgColor} onChange={colorRadioOnChange}/>
              <input name="bgColor" type="color" defaultValue="#FFFFFF" onClick={colorPickerOnClick} />
            </label>
          </dd>
          <dd>
          </dd>
        ...
        <dl>
          ...
          <dd>
            <label>
              <select name="imgType" defaultValue="png" onChange={imgTypeSelectorOnChange}>
          ...
          <dd title="Not applicable to PNG type">
            <label>
              <input type="range" name="imgQuality" min={1} defaultValue={100} disabled />
          ...
          <dd className="through buttons">
            <button action="copy" onClick={saveBtnOnClick} disabled={!settings.textLines}>COPY BASE-64 CODE</button>
            <button action="save" onClick={saveBtnOnClick} disabled={!settings.textLines}>SAVE IMAGE</button>
          </dd>
          <dd className="through note">
            <p>NOTE: If the image is too large to be processed by the browser, the output actions would fail.</p>
          </dd>
        </dl>
      ...
      >
        <RadialText settings={settings} viewportRef={viewportRef} ref={canvasRef} />
      </div>
    </div>
  );
};